var GitHubLocales = function(locale) {
  var locales = {
    "zh-TW": function() {
      /* top nav-bar */
      $("ul.top-nav > li.explore > a").text("瀏覽"); /* Explorer */
      ...
      $("ul.dashboard-tabs > li > a[href*=issues]").text("議案列表"); /* Issues */
      ...
      $("h1.entry-title span.fork-flag > span.text").html("由 " + $("h1.entry-title span.fork-flag > span.text > a")[0].outerHTML + " 分支而來"); /* Forked from {repo_name} */
      ...
      if (window.location.href.match(/^http[s]?:\/\/github.com[\/]+$/) && $("body.logged_in").length) {
        /* right sidebar */
        $("div.dashboard-sidebar h3.box-title:first").text("您有貢獻的專案"); /* Repositories you contribute to */
        $("div.dashboard-sidebar h3.box-title:last").html("您擁有的專案 " + $("div.dashboard-sidebar h3.box-title:last span.box-title-count")[0].outerHTML); /* Your repositories */
      ...
      if (window.location.href.match(/http[s]?:\/\/github.com\/dashboard\/issues/)) {
        ...
        /* filter options */
        $("div.issues-list-options a.minibutton[href*=open]").text("進行中"); /* Open */
      ...
      if (window.location.href.match(/^http[s]?:\/\/github.com\/[^\/]+\/[^\/]+[\/]?$/)) {
        ...
        $("div.overall-summary ul.numbers-summary > li > a[href*=branches]").html($("div.overall-summary ul.numbers-summary > li > a[href*=branches] span.num")[0].outerHTML + " 個旁支"); /* {branches_count} Branches */
        $("div.overall-summary ul.numbers-summary > li > a[href*=releases]").html($("div.overall-summary ul.numbers-summary > li > a[href*=releases] span.num")[0].outerHTML + " 次發佈"); /* {releases_count} Releases */
        ...
        $("div.repository-sidebar div.repo-nav-contents ul.repo-menu li[original-title=Issues] span.full-word").text("議案"); /* Issues */
        ...
        $("div.repository-sidebar div.repo-nav-contents ul.repo-menu li[original-title='Network'] span.full-word").text("旁支總覽"); /* Network */
      ...
      if (window.location.href.match(/http[s]?:\/\/github.com\/[^\/]+\/[^\/]+\/issues/)) {
        /* tab navigation */
        $("div#issues_next > div.tabnav a[href$=issues]").text("議案一覽"); /* Browse Issues */
        $("div#issues_next > div.tabnav a[href$=milestones]").text("里程碑"); /* Milestones */
        $("div#issues_next div.tabnav a[href*=new]").text("提新議案"); /* New Issue */

        console.log("Hello");

        /* left sidebar */
        $("div#issues_next ul.filter-list a[href*=issues]:first").html($("div#issues_next ul.filter-list a[href*=issues]:first span.count")[0].outerHTML + " 所有議案"); /* Everyone's Issues */
        $("div#issues_next ul.filter-list a[href*='issues/milestones'][href*=open]").html($("div#issues_next ul.filter-list a[href*='issues/milestones'][href*=open] span.count")[0].outerHTML + " 進行中里程碑"); /* Open Issues */
        $("div#issues_next ul.filter-list a[href*='issues/milestones'][href*=closed]").html($("div#issues_next ul.filter-list a[href*='issues/milestones'][href*=closed] span.count")[0].outerHTML + " 已結案里程碑"); /* Closed Issues */
